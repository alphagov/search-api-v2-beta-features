...
sequenceDiagram
    ...
    activate GOV.UK
    activate GA4
    Note left of GOV.UK: User interaction data on GOV.UK is<br/>collected using Google Analytics.
...
activate Dataform
Note left of GOV.UK: Dataform runs SQL queries at 1am on the first day of<br/>each month to create implicit "judgement lists" ('binary'<br/>and 'clickstream'), from 3 months of user interaction<br/>data, labelled with a "partition time" of the start of the<br/>month the query is run during.
GA4->>-Dataform: Data from 'partitioned<br/>flattened events table'
Note left of GOV.UK: Example: on 1st November, the judgement list includes<br/>data from August, September and October.
activate Big Query
...
Note left of GOV.UK: A rake task is run at 2am on the first day of<br/>each month to create and import queries into a<br/>single "sample query set", lebelled with a<br/>"partition date" of the start of the month the<br/>query is run during, matching the relevant<br/>judgement list.
